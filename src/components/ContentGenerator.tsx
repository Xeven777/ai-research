...
import { DocumentOutline, generateContent } from '@/utils/documentUtils';
import { ScrollArea } from '@/components/ui/scroll-area';
import { Separator } from '@/components/ui/separator';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { Progress } from '@/components/ui/progress';
import { ArrowLeft, ArrowRight, FileText, RefreshCw, Check } from 'lucide-react';
...
const ContentGenerator: React.FC<ContentGeneratorProps> = ({ 
  ...
  const selectedSubtopics = outline.sections
    ...
    .flatMap(section => section.subtopics.filter(subtopic => subtopic.isSelected));
  ...
  const allContentGenerated = subtopicsWithContent === totalSubtopics && totalSubtopics > 0;
      ...
      <Card className="w-full max-w-4xl mx-auto bg-paper border-0 shadow-md overflow-hidden transition-all duration-400">
        <CardHeader className="bg-primary/5 border-b border-primary/10">
          <div className="flex justify-between items-start">
            <div>
              <CardTitle className="font-serif text-2xl">Generate Content</CardTitle>
              <CardDescription>
                {allContentGenerated 
            ...
            <Button
              ...
              className={cn(
                ...
                allContentGenerated ? "bg-green-600 hover:bg-green-700" : "bg-primary hover:bg-primary/90"
              )}
            >
              {generatingContent ? (
                <>
                          ...
                          <div key={subtopic.id} className="space-y-3">
                            <div className="flex justify-between items-center">
                              ...
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => regenerateSubtopicContent(section.id, subtopic.id)}
                                className="h-8"
                              >
